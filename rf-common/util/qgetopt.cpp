...
GetOpt::GetOpt()
{
    ...
    int n = qApp->arguments().size();
    char** argv;
    argv = new char *[n];
    for (int i=0;i<n;++i)
        argv[i] = (char*)qApp->arguments()[i].toStdString().c_str();

    init( qApp->arguments().size(), argv, 1 );
}
...
GetOpt::GetOpt( int offset )
{
    ...
    init( qApp->arguments().size(), argv, offset );
}
...
bool GetOpt::parse( bool untilFirstSwitchOnly )
{
	...
	if ( t != End ) {
		...
		if ( currOpt.type != OVarLen ) {
            qWarning( "Unknown option --%s",a.toUtf8().constData() );
		    return false;
		} else {
	...
	case ExpectingState:
        ...
        qWarning( "Expected an argument after '%s' option", n.toUtf8().constData() );
		return false;
	    }
	    break;
	case OptionalState:
